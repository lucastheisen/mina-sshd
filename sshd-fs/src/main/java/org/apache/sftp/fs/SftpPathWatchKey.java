...
public class SftpPathWatchKey implements WatchKey, Runnable {
    private static Logger logger = LoggerFactory.getLogger( SftpPathWatchKey.class );
    private Map<SftpPath, SftpPathWatchEvent<Path>> addMap;
    private boolean cancelled;
    private Map<SftpPath, SftpPathWatchEvent<Path>> deleteMap;
    private SftpPath dir;
    private Map<SftpPath, PosixFileAttributes> entries;
    private boolean initialized;
    private Set<Kind<?>> kindsToWatch;
    private Map<SftpPath, SftpPathWatchEvent<Path>> modifyMap;
    private long pollingInterval;
    private TimeUnit pollingIntervalTimeUnit;
    private State state;
    private SftpFileSystemWatchService watchService;
    ...
    public SftpPathWatchKey( SftpFileSystemWatchService watchService, SftpPath dir, Kind<?>[] kinds, long pollingInterval, TimeUnit pollingIntervalTimeUnit ) {
    ...
    void addCreateEvent( SftpPath path ) {
        if ( kindsToWatch.contains( StandardWatchEventKinds.ENTRY_CREATE ) ) {
            try {
                mapLock.lock();
                logger.trace( "added: {}", path );
                if ( !addMap.containsKey( path ) ) {
                    addMap.put( path, new SftpPathWatchEvent<Path>( StandardWatchEventKinds.ENTRY_CREATE, path ) );
                }
            }
            finally {
                mapLock.unlock();
    ...
    void addDeleteEvent( SftpPath path ) {
        if ( kindsToWatch.contains( StandardWatchEventKinds.ENTRY_DELETE ) ) {
            try {
                mapLock.lock();
                logger.trace( "deleted: {}", path );
                if ( !deleteMap.containsKey( path ) ) {
                    deleteMap.put( path, new SftpPathWatchEvent<Path>( StandardWatchEventKinds.ENTRY_DELETE, path ) );
                }
            }
            finally {
                mapLock.unlock();
    ...
    void addModifyEvent( SftpPath path ) {
        if ( kindsToWatch.contains( StandardWatchEventKinds.ENTRY_MODIFY ) ) {
            try {
                ...
                if ( modifyMap.containsKey( path ) ) {
                    ...
                    SftpPathWatchEvent<Path> event = new SftpPathWatchEvent<Path>( StandardWatchEventKinds.ENTRY_MODIFY, path );
        ...
        try {
            while ( true ) {
                ...
                try {
                    ...
                    Map<SftpPath, PosixFileAttributes> entries =
                    ...
                    if ( first ) {
                        ...
                        for ( SftpPath entryPath : entries.keySet() ) {
                        ...
                        for ( SftpPath entryPath : this.entries.keySet() ) {
